...
public class DriveTemplateTests extends AbstractGoogleApiTest
{
    ...
    public void getAbout()
    {
        ...
        mockServer.expect(requestTo("https://www.googleapis.com/drive/v2/about"))
                  .andExpect(method(GET))
                  .andRespond(withSuccess(jsonResource("about"), APPLICATION_JSON));
        ...
        assertEquals("https://lh5.googleusercontent.com/-UyuMuAWmKIM/AAAAAAAAAAI/AAAAAAAAAn0/pMK2DzFNBNI/s64/photo.jpg",
    ...
    public void createFileMetadata()
    {
        ...
        mockServer.expect(requestTo("https://www.googleapis.com/drive/v2/files/"))
                  .andExpect(method(POST))
                  .andRespond(withSuccess(jsonResource("file"), APPLICATION_JSON));

        DriveFile file = google.driveOperations()
                               .createFileMetadata(DriveFile.builder()
                                                            .setTitle("newfile")
                                                            .setDescription("A New File")
                                                            .build());
        ...
        assertEquals("\"sIP8ArR2PAy9qIx8FYkTpbHmKik/MTQwMDQ4NDIzNzc3OQ\"", file.getEtag());
        assertEquals("https://content.googleapis.com/drive/v2/files/0B3DS_k_gCihOVXpISDlScnJpTFk", file.getSelfLink());
        assertEquals("https://docs.google.com/file/d/0B3DS_k_gCihOVXpISDlScnJpTFk/edit?usp=drivesdk",
                     file.getAlternateLink());
        assertEquals("https://ssl.gstatic.com/docs/doclist/images/icon_10_generic_list.png", file.getIconLink());
        assertFalse(file.isStarred());
        assertFalse(file.isHidden());
        assertFalse(file.isTrashed());
        assertFalse(file.isRestricted());
        assertTrue(file.isViewed());
        assertEquals("https://doc-0s-4c-docs.googleusercontent.com/docs/securesc/onj78cqef11u939b14bgc0mu1vntl04c/1mpjiafeocj32omvpv6augb74kl0hge3/1400479200000/09558676735003532639/09558676735003532639/0B3DS_k_gCihOVXpISDlScnJpTFk?h=16653014193614665626&e=download&gd=true",
                     file.getDownloadUrl());

        assertNotNull(file.getUserPermission());
        assertEquals("me", file.getUserPermission().getId());
        assertEquals("\"sIP8ArR2PAy9qIx8FYkTpbHmKik/VACuEBTone13WaJiFZKx-FuSPpA\"", file.getUserPermission().getEtag());
    ...
    public void deleteFile()
    {
        ...
        mockServer.expect(requestTo("https://www.googleapis.com/drive/v2/files/0B3DS_k_gCihOVXpISDlScnJpTFk"))
